...
1. **Non-portable Windows test (finding #1).** `tests/test_utils.py::TestEnsureFfmpegOnPath::test_windows_uses_exe_name_and_copy_fallback` monkeypatches `sys.platform="win32"` but isn't skipped off-Windows; under Python 3.12 on Linux, stdlib `shutil.which` calls `_winapi.NeedCurrentDirectoryForExePath` and crashes (`_winapi` is `None`). Passes on the project's Windows + Python 3.9 target; fails in any non-Windows CI/sandbox. Low severity — needs a `@pytest.mark.skipif(sys.platform != "win32")` guard or a `_winapi` mock. See DECISIONS D-002.
